...
const roundDecimals = (arr) => {
    let res=[];
    arr.forEach((decimal,index)=>{
        res[index]=Math.round(decimal);
    });
    return res;
}
...
const employeesBonus = (arr) => {
    ...
    arr.forEach((element,index,array)=>{
        if(array[index].workHours>8)
        {
            newSal= parseInt(array[index].salary)+100;
            array[index].salary=`${newSal}$`;
        }
        else{
            newSal= parseInt(array[index].salary)+50;
            array[index].salary=`${newSal}$`;
        }
    });
    return arr;
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    let max=0;
    mouseArray.forEach((num,i,array)=>{
        keyBoardArray.forEach((numK,iK,arrayK)=>{
            if(num+numK>max && num+numK<=budget)
            {
                max=num+numK;
            }
        });
