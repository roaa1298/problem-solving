...
const objLat = (obj) => {
    let line=`my name is ${obj.firstName.charAt(0).toUpperCase()}${obj.firstName.slice(1)} ${obj.lastName.charAt(0).toUpperCase()}${obj.lastName.slice(1)} I am ${obj.age} YO, and I love ${obj.hobby}.`;
    return line;
};
...
const cvFormatter = (arr) => {
    let arr2;
    let arr3;
    arr2=arr.filter(test=>test.yearsOfExperience>1);
    arr3=arr2.map(ex=>{
        let prop={
            "fullName": `${ex.firstName} ${ex.lastName}` ,
            "tech": ex.tech
        };
        if(ex.lastName==null)
        prop.fullName=ex.firstName;
